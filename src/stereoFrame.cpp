...
StereoFrame::StereoFrame(const Mat img_l_, const Mat img_r_ , const int idx_, PinholeStereoCamera *cam_) :
    ...
    inv_width  = GRID_COLS / static_cast<double>(img_l.cols);   // 每一列（行）为多少个栅格列（行）
...
// 提取图像点线特征进行匹配，得到带深度的点特征stereo_pt 和描述子 pdesc_l，线段特征 stereo_ls 和描述子ldesc_l 
void StereoFrame::extractStereoFeatures( double llength_th, int fast_th )
{

    if( Config::plInParallel() )
    {
        auto detect_p = async(launch::async, &StereoFrame::detectStereoPoints,        this, fast_th );      // 多线程异步操作
...
// 检测双目图像ORB特征点和描述子，并且进行双目匹配，得到带深度的点特征 stereo_pt 和描述子 pdesc_l
...
void StereoFrame::detectPointFeatures( Mat img, vector<KeyPoint> &points, Mat &pdesc, int fast_th )
{
    ...
    if( Config::hasPoints() )
    {
        ...
        Ptr<ORB> orb = ORB::create( Config::orbNFeatures(), Config::orbScaleFactor(), Config::orbNLevels(),
                                    Config::orbEdgeTh(), 0, Config::orbWtaK(), ORB::ScoreType( Config::orbScore() ),
...
// 左右目特征匹配，计算正确匹配的特征点的深度，归一化坐标，并更新描述子 pdesc_l_
void StereoFrame::matchStereoPoints( vector<KeyPoint> points_l, vector<KeyPoint> points_r, Mat &pdesc_l_, Mat pdesc_r, bool initial )
{
    ...
    for (const KeyPoint &kp : points_l)     // 计算特征点在网格中的坐标
        coords.push_back(std::make_pair(kp.pt.x * inv_width, kp.pt.y * inv_height));

    //Fill in grid
    GridStructure grid(GRID_ROWS, GRID_COLS);       // 将右目特征点保存在网格容器中
    ...
    GridWindow w;
    w.width = std::make_pair(Config::matchingSWs(), 0);     // 左目图像中观察到的特征点，在右目图像中靠左
    w.height = std::make_pair(0, 0);    // 这里搜索的网格高度为1，实际应用时要修改，不然在网格边缘的特征点很可能就不匹配了

    std::vector<int> matches_12;
    matchGrid(coords, pdesc_l, grid, pdesc_r, w, matches_12);   // 得到双向最佳匹配 matches_12
//    match(pdesc_l, pdesc_r, Config::minRatio12P(), matches_12);

    // bucle around pmatches
    Mat pdesc_l_aux;    // 保留左目匹配的描述子
    int pt_idx = 0;
    for (int i1 = 0; i1 < matches_12.size(); ++i1) {
        ...
        if (std::abs(points_l[i1].pt.y - points_r[i2].pt.y) <= Config::maxDistEpip()) {     // 如果 y 坐标相差过大，则匹配无效
            // check minimal disparity
            double disp_ = points_l[i1].pt.x - points_r[i2].pt.x;
            if (disp_ >= Config::minDisp()){        // 特征点的 x 坐标差要大于阈值
                pdesc_l_aux.push_back(pdesc_l_.row(i1));
                Vector2d pl_(points_l[i1].pt.x, points_l[i1].pt.y);
                Vector3d P_ = cam->backProjection(pl_(0), pl_(1), disp_);   // 反投影得到归一化平面坐标 P_ 和 特征点深度 disp_
                if (initial)    // 如果是初始帧，累加特征的id；否则将特征的id都置为-1
...
// 检测双目图像中的线段，计算描述子进行匹配，得到带深度的线段特征 stereo_ls 和描述子ldesc_l
...
// 使用作者修改过的FLD和LSD提取线段和计算描述子
void StereoFrame::detectLineFeatures( Mat img, vector<KeyLine> &lines, Mat &ldesc, double min_line_length )
{

    // Detect line features
    lines.clear();
    Ptr<BinaryDescriptor>   lbd = BinaryDescriptor::createBinaryDescriptor();   // 这里是线段二进制描述子
    if( Config::hasLines() )
    {
        
        if( !Config::useFLDLines() )    // 如果不使用快速线段检测(FLD)
        {
            ...
            lsd->detect( img, lines, Config::lsdScale(), 1, opts);  // 只在原始图像上提取线段特征

            // filter lines
            if( lines.size()>Config::lsdNFeatures() && Config::lsdNFeatures()!=0  )     // 如果提取的线段数量过多，选择响应靠前的线段
            {
        ...
        else
        {
            ...
            {
                // sort lines by their response
                sort( fld_lines.begin(), fld_lines.end(), sort_flines_by_length() );    // 根据长度进行排序
...
void StereoFrame::matchStereoLines( vector<KeyLine> lines_l, vector<KeyLine> lines_r, Mat &ldesc_l_, Mat ldesc_r, bool initial )
{
    ...
    list<pair<int, int>> line_coords;       // 保存右目线段所有点的容器
    GridStructure grid(GRID_ROWS, GRID_COLS);   // 每个网格保存经过该网格的右目中线段的索引
    std::vector<std::pair<double, double>> directions(lines_r.size());  // [delta x, delta y] 单位向量 容器
    for (int idx = 0; idx < lines_r.size(); ++idx) {
        const KeyLine &kl = lines_r[idx];

        std::pair<double, double> &v = directions[idx];     // [delta x, delta y] 单位向量
        v = std::make_pair((kl.endPointX - kl.startPointX) * inv_width, (kl.endPointY - kl.startPointY) * inv_height);  
        normalize(v);

        getLineCoords(kl.startPointX * inv_width, kl.startPointY * inv_height, kl.endPointX * inv_width, kl.endPointY * inv_height, line_coords);   // 得到线段上所有的点，保存到 line_coords中
        for (const std::pair<int, int> &p : line_coords)
            grid.at(p.first, p.second).push_back(idx);
    }

    GridWindow w;
    w.width = std::make_pair(Config::matchingSWs(), 0);     // 左目图像中观察到的线段，在右目图像靠左
    w.height = std::make_pair(0, 0);

    std::vector<int> matches_12;
    matchGrid(coords, ldesc_l, grid, ldesc_r, directions, w, matches_12);   // 得到线段匹配关系 matches_12
//    match(ldesc_l, ldesc_r, Config::minRatio12P(), matches_12);

    // bucle around lmatches
    Mat ldesc_l_aux;    // 保存匹配上的线段的描述子
    int ls_idx = 0;
    for (int i1 = 0; i1 < matches_12.size(); ++i1) {
        const int i2 = matches_12[i1];
        if (i2 < 0) continue;
        ...
        Vector3d le_l; le_l << sp_l.cross(ep_l); le_l = le_l / std::sqrt( le_l(0)*le_l(0) + le_l(1)*le_l(1) );      // 左目线段参数（归一化）
        ...
        Vector3d le_r; le_r << sp_r.cross(ep_r);    // 右目线段参数（未归一化）

        double overlap = lineSegmentOverlapStereo( sp_l(1), ep_l(1), sp_r(1), ep_r(1) );    // 如果是水平线，重叠程度为1；否则，计算线段的重叠程度

        double disp_s, disp_e;
        // 我真是个天才，看懂了下面两行代码
        // 其实就是把右目线段进行长度调整使得两个端点的y坐标和左目线段一致
        ...
        filterLineSegmentDisparity( sp_l.head(2), ep_l.head(2), sp_r.head(2), ep_r.head(2), disp_s, disp_e );   // 如果x坐标差值过大，差值设为 -1

        // check minimal disparity
        if( disp_s >= Config::minDisp() && disp_e >= Config::minDisp()      // x坐标差值大于阈值
            && std::abs( sp_l(1)-ep_l(1) ) > Config::lineHorizTh()          // 左右目线段的y坐标差值大于阈值，即线段非水平
            && std::abs( sp_r(1)-ep_r(1) ) > Config::lineHorizTh()
            && overlap > Config::stereoOverlapTh() )        // 线段的重叠程度大于阈值
        {
            Vector3d sP_; sP_ = cam->backProjection( sp_l(0), sp_l(1), disp_s);     // 计算左目线段两端点的归一化坐标和深度值   
...
{
    disp_s = spl(0) - spr(0);   // 两线段起始点 x坐标差值
    disp_e = epl(0) - epr(0);   // 两线段终点   y坐标差值
...
double StereoFrame::lineSegmentOverlapStereo( double spl_obs, double epl_obs, double spl_proj, double epl_proj  )
{
    ...
    if( fabs( epl_obs - spl_obs ) > Config::lineHorizTh() ) // normal lines (verticals included)    // 左目图像线段起始点的 y 的差值大于阈值，说明不是水平线
    {
        ...
        if ( (epn < sln) || (spn > eln) )   // 两线段一左一右，没有重叠
            overlap = 0.f;
        else{
            if ( (epn>eln) && (spn<sln) )   // 右目图像观察到的线段 覆盖了 左目图像观察到的线段
                overlap = eln-sln;          // 重叠部分长度为左目线段的长度
            else
                overlap = min(eln,epn) - max(sln,spn);  // 取重叠长度
        }
...
Mat StereoFrame::plotStereoFrame()
{
    ...
    if( img_l_aux.channels() == 1 )
        cvtColor(img_l_aux, img_l_aux, COLOR_GRAY2BGR, 3);
