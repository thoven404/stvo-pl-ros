...
void StereoFrameHandler::initialize(const Mat img_l_, const Mat img_r_ , const int idx_)
{
    ...
    prev_frame->extractStereoFeatures( llength_th, orb_fast_th );   // 提取点线特征
...
void StereoFrameHandler::matchF2FPoints()
{
    ...
    // points f2f tracking
    // --------------------------------------------------------------------------------------------------------------------
    matched_pt.clear();     // 保存上一帧匹配上当前帧的特征点
    ...
    for (int i1 = 0; i1 < matches_12.size(); ++i1) {
        ...
        prev_frame->stereo_pt[i1]->pl_obs = curr_frame->stereo_pt[i2]->pl;  // 上一帧pl_obs保存当前帧对应特征点的二维坐标
        prev_frame->stereo_pt[i1]->inlier = true;                           // 匹配成功，标记为内点
        matched_pt.push_back( prev_frame->stereo_pt[i1]->safeCopy() );      // 搞不懂为什么要new新的内存空间，不会造成内存泄露吗？（哦，这里list保存的是指针）
...
void StereoFrameHandler::matchF2FLines()
{
    ...
    // match(prev_frame->ldesc_l, curr_frame->ldesc_l, 1.0, matches_12);
...
void StereoFrameHandler::optimizePose()
{
    ...
    {
        DT     = prev_frame->DT;        // Matrix4d::Identity();
        DT_cov = prev_frame->DT_cov;    // 空
        e_prev = prev_frame->err_norm;  // 空
