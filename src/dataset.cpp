...
Dataset::Dataset(const std::string &dataset_path, const PinholeStereoCamera &cam, int offset, int nmax, int step)
    ...
    boost::filesystem::path dataset_params = dataset_base / "euroc_params.yaml";
    ...
    boost::regex expression("^[^0-9]*([0-9]+\\.?+[0-9]*)[^0-9]*\\.[a-z]{3,4}$");    // 正则表达式
    boost::cmatch what;
...
bool Dataset::nextFrame(cv::Mat &img_l, cv::Mat &img_r) {
    ...
    img_l = cv::imread(images_l.front(), IMREAD_UNCHANGED);  // 列表头读取图片
